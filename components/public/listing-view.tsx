...
// The guest-facing property page — shared by the public route (app/[slug], cached/ISR) and the
// admin preview route (app/(admin)/admin/preview/[slug], dynamic). Keeping the render here lets the
// public page stay cookie-free (hence cacheable) while admins can still vet an unapproved listing.
export type Listing = {
  property: {
    id: string;
    name: string;
...
export const EMPTY_REVIEWS: ReviewsData = { avg_rating: null, review_count: 0, reviews: [] };

// Resolve a raw listing's storage paths to public image URLs. Shared by both routes so the two
// stay pixel-identical. Public URLs need no auth, so either the anon or the authed client works.
export function resolveListingAssets(listing: Listing, supabase: SupabaseClient<Database>) {
  ...
  return { coverUrl, ogImageUrl, rooms, spacePhotos };
}
