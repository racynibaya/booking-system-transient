...
// Shared PayMongo webhook body — everything AFTER signature verification. Both the flat 2a route
// (env secret) and the per-tenant M3 route (the connection's own whsk_) verify with their own secret
// and then hand the already-verified raw body here, so the confirm contract, idempotency, and
// 200/4xx/5xx classification live in ONE place and can never fork (architecture P7).
//
// We ALWAYS return 200 for a well-formed event (handled, ignored, or idempotent no-op) so PayMongo
// stops retrying; only malformed shape (400) or a transient confirm failure (500) is non-200.
export async function handleVerifiedPaymongoEvent(rawBody: string): Promise<Response> {
