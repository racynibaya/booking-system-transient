-- F1.8 — Per-property social links (Facebook / Instagram / TikTok). All additive: three nullable
-- url columns + the same keys on the public RPC. Socials are public by nature (unlike GCash, which
-- stays off the listing), so exposing them on get_public_listing is fine.
alter table public.properties
  add column facebook_url  text,
  add column instagram_url text,
  add column tiktok_url    text;

-- get_public_listing — MODIFIED: expose the three social URLs. Body otherwise identical to F1.7
-- (20260618140000_listing_amenities). Additive keys only; signature, language/volatility, security,
-- and the anon execute grant are unchanged.
